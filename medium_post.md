...
In conclusion, we have successfully developed an interactive medical image interpretation platform that aids patients in better understanding their chest X-ray diagnosis. We take immense pride in our progress and are grateful for the invaluable assistance from the teaching staff. Our learning journey has encompassed various aspects, including containerization (Docker), version control (Git), deployment (Ansible, Kubernetes), experiment tracking (wandb), web development (FastAPI, React), and cloud computing platforms (GCP, Vertex AI).

Throughout the development process, we encountered several challenges related to Docker, Ansible, PyTorch, and Vite. However, through diligent efforts and the generous help from the teaching staff, we were able to overcome these obstacles.
